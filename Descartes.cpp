...
float CartesianPoint::slope(const CartesianPoint& p) const 
{
	...
	float slope;
	slope = ((p._y)-(this->_y))/((p._x)-(this->_x));
	return slope;
}

float CartesianPoint::distance(const CartesianPoint& p) const
{   
	float val = ((p._y - this->_y)*(p._y - this->_y)) + ((p._x - this->_x)*(p._x - this->_x)); 
	float Dist = sqrt(val);
	return Dist;
}
...
{
	float s1 = sqrt(((p1._y - this->_y)*(p1._y - this->_y)) + ((p1._x - this->_x)*(p1._x - this->_x)));
	float s2 = sqrt(((p2._y - p1._y)*(p2._y - p1._y)) + ((p2._x - p1._x)*(p2._x - p1._x)));
	float s3 = sqrt(((this->_y - p2._y)*(this->_y - p2._y)) + ((this->_x - p2._x)*(this->_x - p2._x)));
	if( s1 == 0 && s2 == 0 && s3 == 0)
		return 0;
	if( s1 == 0 || s2 == 0 || s3 == 0)
		return 0;
	float s = 0.5* (s1+s2+s3);
	float val = s*(s-s1)*(s-s2)*(s-s3);
	return sqrt(val);
}

bool  CartesianPoint::straightLine(const CartesianPoint& p1, const CartesianPoint& p2) const 
{
	float STP = ((this->_x)*(p1._y - p2._y)) - ((this->_y)*(p1._x - p2._x)) + ((p1._x)*(p2._y))-((p1._y)*(p2._x));
	if( STP == 0)
	{
	   return true;
	}
